...
func (ef *ErrorFactory) NewEmptyOrgError() *SBOMExtensionError {
	return ef.newErr(
		...
		"Vulnmap failed to infer an organization ID. Please make sure to authenticate using `khulnasoft-lab auth`. "+
...
func (ef *ErrorFactory) NewUnauthorizedError(err error) *SBOMExtensionError {
	return ef.newErr(
		err,
		"Vulnmap failed to authenticate you based on on you API token. "+
	...
	return ef.newErr(
		...
		fmt.Sprintf(
			"Your account is not authorized to perform this action. "+
				...
				"the organization is entitled to use the Vulnmap API. (Org ID: %s)",
			orgID,
		),
	)
}
